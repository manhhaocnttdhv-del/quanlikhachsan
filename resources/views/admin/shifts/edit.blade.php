...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h2><i class="fas fa-calendar-edit"></i> Chỉnh sửa ca làm việc</h2>
        ...
        <form action="{{ route('admin.shifts.update', $shift->id) }}" method="POST">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select name="admin_id" class="form-select @error('admin_id') is-invalid @enderror" required>
                        ...
                        @foreach($employees as $employee)
                            <option value="{{ $employee->id }}" {{ old('admin_id', $shift->admin_id) == $employee->id ? 'selected' : '' }}>
                                {{ $employee->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <select name="shift_type" id="shiftTypeSelect" class="form-select @error('shift_type') is-invalid @enderror" required>
                        ...
                        <option value="morning" {{ old('shift_type', $shift->shift_type) == 'morning' ? 'selected' : '' }}>Ca sáng (06:00 - 12:00)</option>
                        <option value="afternoon" {{ old('shift_type', $shift->shift_type) == 'afternoon' ? 'selected' : '' }}>Ca trưa (12:00 - 18:00)</option>
                        <option value="evening" {{ old('shift_type', $shift->shift_type) == 'evening' ? 'selected' : '' }}>Ca tối (18:00 - 24:00)</option>
                    </select>
                    @error('shift_type')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <select name="status" class="form-select @error('status') is-invalid @enderror" required>
                        <option value="scheduled" {{ old('status', $shift->status) == 'scheduled' ? 'selected' : '' }}>Đã lên lịch</option>
                        <option value="active" {{ old('status', $shift->status) == 'active' ? 'selected' : '' }}>Đang làm</option>
                        <option value="completed" {{ old('status', $shift->status) == 'completed' ? 'selected' : '' }}>Hoàn thành</option>
                        <option value="cancelled" {{ old('status', $shift->status) == 'cancelled' ? 'selected' : '' }}>Đã hủy</option>
                    </select>
                    @error('status')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
            ...
            <div class="mb-3">
                ...
                <textarea name="notes" class="form-control @error('notes') is-invalid @enderror" rows="3" 
                          placeholder="Ghi chú về ca làm việc...">{{ old('notes', $shift->notes) }}</textarea>
                @error('notes')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
